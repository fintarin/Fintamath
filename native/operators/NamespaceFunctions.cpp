...
//  Округление
void round(Fraction &a, const size_t precision) { a.round(PRECISION_OF_FUNCTIONS); }
...
/*
   Уменьшение значение a под логарифмом так, чтобы a -> 1, поскольку в
   окрестности данной точке ряд Тейлора быстро сходится. Используя формулу
   log(a^n) = n*log, путем множественного взятия квадратного корня, число
   приводится к требуемуму виду. После расчета логарифма получившегося числа,
   результат домножается на 2^n, где n — количество опреаций взятия корня.
*/
...
/*
    Вычисление натурального ln(a), используется ряд Тейлора:
    ln(a) = sum_{k=0}^{inf} (2/(2k+1)) * ((a-1)/(a+1))^(2k+1)
*/
...
/*
    Возведение действительного a в действительную степень n.
    n можно представить в виде n_int + n_float, где |n_float| <= 1, тогда a^n =
   a^n_int * a^n_float. Для вычисления a^n_float, используется ряд Тейлора: a^n
   = 1 + sum_{k=1}^{inf} (n * ln(a))^k / k! при |n| <= 1.
*/
...
/*
    Уменьшение значения a под тригонометрической функцией.
    Т.к. тригонометрические функции являются периодическими,
    для них верно следующее: f(a) = f(b + k*p) = f(b), где k — натуральное
   число, p - период функции. Тогда b = a - k*period, k = a div p.
*/
...
/*
    Вычисление sin(a), используется ряд Тейлора:
    sin(a) = sum_{k=0}^{k=1} (-1)^k * x^(2k+1) / (2k+1)!
    Для большего сокращения a используются формулы приведения.
*/
...
/*
    Вычисление cos(a), используется ряд Тейлора:
    sin(a) = sum_{k=0}^{k=1} (-1)^k * x^(2k) / (2k)!
    Для большего сокращения a используются формулы приведения.
*/
...
/*
    ...
    При |a| <= 1/5 используется ряд Тейлора (при данных значениях он быстро
   сходится): acos(a) = pi/2 - sum_{k=0}^{inf} ((2k)! * a^(2k+1) / (4^k * (k!)^2
   * (2k+1)).

    При других значениях используется формула:
    acos(a) = 2atan(sqrt((1-x)/(1+x))).
*/
...
/*
    ...
    При |a| <= 1/5 используется ряд Тейлора (при данных значениях он быстро
   сходится): acos(a) = sum_{k=1}^{inf} ((-1)^(k-1) * a^(2k-1)) / ((2k-1)).

    При других значениях используется формула:
    acos(a) = acos(1 / sqrt(1 + x^2)).
*/
...
/*
   Вычисление факториала через разложение множителей в дерево.
   Такое вычисление факториала быстрее, чем наивный алгоритм,
   т.к. умножать большие числа примерно равной длины гораздо быстрее, чем
   большие числа на маленькие. Это обусловлено использованием алгоритма
   Карацубы.
*/
