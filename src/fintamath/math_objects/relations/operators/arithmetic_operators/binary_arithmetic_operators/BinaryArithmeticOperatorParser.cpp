...
namespace fintamath {
  ...
  std::unique_ptr<Expression> BinaryArithmeticOperatorParser::parse(const std::string_view &str) {
    ...
    for (int i = (int)str.size() - 1; i > 0; i--) {
      ...
      if (str[i - 1] == plusSign || str[i - 1] == minusSign || str[i - 1] == divideSign || str[i - 1] == multiplySign ||
          (str[i] != plusSign && str[i] != minusSign)) {
        continue;
      }
    ...
    for (int i = (int)str.size() - 1; i >= 0; i--) {
      ...
      if (i == str.size() - 1 || i == 0 || str[i - 1] == plusSign || str[i - 1] == minusSign ||
          str[i - 1] == divideSign || str[i - 1] == multiplySign) {
        throw std::invalid_argument("Expression invalid input");
      }
