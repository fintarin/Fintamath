// #include "fintamath/functions/FunctionComparator.hpp"
...
// Ordering compareNonExpressions(const SharedRef<IMathObject> &lhs, const SharedRef<IMathObject> &rhs, ComparatorOptions options);
...
// Ordering comparePolynomAndNonPolynom(const PolynomPtr &lhs, const SharedRef<IMathObject> &rhs, ComparatorOptions options);

// Ordering compareExpressionAndNonExpression(const ExpressionPtr &lhs, const SharedRef<IMathObject> &rhs, ComparatorOptions options);

// Ordering compareFunctions(const FunctionPtr &lhs, const FunctionPtr &rhs);

// ChildrenComparatorResult compareChildren(const Arguments &lhsChildren, const Arguments &rhsChildren, ComparatorOptions options);

// bool unwrapEmptyExpression(SharedRef<IMathObject> &arg);

// Ordering reverse(Ordering ordering);

// template <typename Term>
// size_t getPositionOfFirstChildWithTerm(const Arguments &children) {
//   for (size_t i = 0; i < children.size(); i++) {
//     if (containsIf(children[i], [](const SharedRef<IMathObject> &child) { return is<Term>(child); })) {
//       return i;
//     }
//   }
...
//   while (!stack.empty()) {
//     const Arguments &children = stack.top().first->getChildren();
...
// template <typename Term>
// Ordering compareTerms(const SharedRef<IMathObject> &lhs, const SharedRef<IMathObject> &rhs, const ComparatorOptions options) {
...
// Ordering compare(SharedRef<IMathObject> lhs, SharedRef<IMathObject> rhs, const ComparatorOptions options) {
...
// Ordering compareNonExpressions(const SharedRef<IMathObject> &lhs, const SharedRef<IMathObject> &rhs, const ComparatorOptions options) {
...
//   Arguments lhsChildren = lhs->getChildren();
//   Arguments rhsChildren = rhs->getChildren();
...
// Ordering comparePolynomAndNonPolynom(const PolynomPtr &lhs, const SharedRef<IMathObject> &rhs, const ComparatorOptions options) {
...
// Ordering compareExpressionAndNonExpression(const ExpressionPtr &lhs, const SharedRef<IMathObject> &rhs, const ComparatorOptions options) {
//   if (!is<ILiteral>(rhs)) {
//     return !options.termOrderInversed ? Ordering::greater : Ordering::less;
//   }
...
//     const SharedRef<IMathObject> rhsExpr = makeExpr(*lhs->getFunction(), rhs, Integer(1).clone());
...
// ChildrenComparatorResult compareChildren(const Arguments &lhsChildren, const Arguments &rhsChildren, const ComparatorOptions options) {
