...
TEST(IIntegerTests, modTest) {
  ...
  EXPECT_THAT(
      ...
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Invalid arguments of "%" operator (Integer "10" and TestInteger "test" are unconvertible to each other))")));
  EXPECT_THAT(
      [&] { TestInteger() % *m1; },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Invalid arguments of "%" operator (TestInteger "test" and Integer "10" are unconvertible to each other))")));
