...
TEST(RealTests, stringConstructorTest) {
  ...
  EXPECT_THAT(
      ...
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "")")));
  EXPECT_THAT(
      [] { Real("--10"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "--10")")));
  EXPECT_THAT(
      [] { Real("test"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "test")")));
  EXPECT_THAT(
      [] { Real("+"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "+")")));
  EXPECT_THAT(
      [] { Real("939849.0-0023"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "939849.0-0023")")));
  EXPECT_THAT(
      [] { Real("a"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "a")")));
  EXPECT_THAT(
      [] { Real("a.1"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "a.1")")));
  EXPECT_THAT(
      [] { Real("1.a"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "1.a")")));
  EXPECT_THAT(
      [] { Real("1a.1"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "1a.1")")));
  EXPECT_THAT(
      [] { Real("1.1a"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "1.1a")")));
  EXPECT_THAT(
      [] { Real("--10.-1"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "--10.-1")")));
  EXPECT_THAT(
      [] { Real("10.-1"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "10.-1")")));
  EXPECT_THAT(
      [] { Real("1-0.1"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "1-0.1")")));
  EXPECT_THAT(
      [] { Real("10-.1"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "10-.1")")));
  ...
  EXPECT_THAT(
      [] { Real("1.2.1"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "1.2.1")")));
  EXPECT_THAT(
      [] { Real("2*10^2.2"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "2*10^2.2")")));
  EXPECT_THAT(
      [] { Real("2*10^-2.2"); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Unable to parse a Real from "2*10^-2.2")")));
...
TEST(RealTests, toStringPrecisionPrecisionTest) {
  ...
  EXPECT_THAT(
      [&] { val.toString(20); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(The given precision 20 is greater than the current precision 10)")));
}
...
TEST(RealTests, setOutputPrecisionTest) {
  ...
  EXPECT_THAT(
      ...
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq("The given precision 6 is greater than the current precision 5")));
  EXPECT_THAT(
      [&] { a.setOutputPrecision(10); },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq("The given precision 10 is greater than the current precision 5")));
}
