...
TEST(ExpressionTests, stringConstructorTest) {
  ...
  EXPECT_EQ(Expression("ComplexInf*-Inf").toString(), "ComplexInf");
  EXPECT_EQ(Expression("a * Inf").toString(), "Inf sign(a)");
  EXPECT_EQ(Expression("Inf * (Inf x)").toString(), "Inf sign(x)");
  ...
  EXPECT_EQ(Expression("-Inf <= x").toString(), "-Inf <= x");
  EXPECT_EQ(Expression("-Inf >= x").toString(), "-Inf >= x");
  EXPECT_EQ(Expression("x Inf = Inf").toString(), "Inf sign(x) = Inf");
  EXPECT_EQ(Expression("Inf = x Inf").toString(), "Inf = Inf sign(x)");
  EXPECT_EQ(Expression("x Inf < Inf").toString(), "Inf sign(x) < Inf");
  EXPECT_EQ(Expression("Inf < x Inf").toString(), "Inf < Inf sign(x)");
  EXPECT_EQ(Expression("x Inf = 0").toString(), "Inf sign(x) = 0");
  EXPECT_EQ(Expression("0 = x Inf").toString(), "0 = Inf sign(x)");
  EXPECT_EQ(Expression("x Inf < 0").toString(), "Inf sign(x) < 0");
  EXPECT_EQ(Expression("0 < x Inf").toString(), "0 < Inf sign(x)");
