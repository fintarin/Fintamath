...
TEST(IArithmeticTests, addTest) {
  ...
  EXPECT_THAT(
      ...
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Invalid arguments of "+" operator (Integer "1" and TestArithmetic "TestArithmetic" are unconvertible to each other))")));
  EXPECT_THAT(
      [&] { TestArithmetic() + *m1; },
      testing::ThrowsMessage<InvalidInputException>(
          testing::StrEq(R"(Invalid arguments of "+" operator (TestArithmetic "TestArithmetic" and Integer "1" are unconvertible to each other))")));
